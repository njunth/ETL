...
TABLE_NAMES = ['statistic_t','sentiment_t','distribution_t']
...
for row in keywords:
    ...
    for i in range(4):
        ...
        for j in range(10):
            ...
            print(result)
            insert_sql = "REPLACE INTO statistic_t VALUES('"+keyword+"','"+timestr2+"','"+SITE_TYPES[i]+"',"+str(result['hits']['total'])+")"
    ...
    for i in range(4):
        ...
        result = es.search(index='crawler', body=body, doc_type=SITES[SITE_TYPES[i]],request_timeout=30)
        print(result)
        insert_sql = "REPLACE INTO distribution_t VALUES('" + keyword + "','" + SITE_TYPES[i] + "'," + str(result['hits']['total']) + ")"
        cursor.execute(insert_sql)
        print(insert_sql)
